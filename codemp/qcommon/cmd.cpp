...
#define	MAX_CMD_BUFFER	262144 //128*2048 //128*1024
...
static void Cmd_Exec_f( void ) {
	...
	if (!Q_stricmpn(Cmd_Argv(1), "jampconfig", 10))
		return; //No.
	...
	if (!f.c) {
		if (!quiet) Com_Printf ("couldn't exec %s\n", filename);
		return;
	}
	if (!quiet)
		Com_Printf ("execing %s\n", filename);

#ifndef DEDICATED //TESTMETESTME??
	if (com_cl_running && com_cl_running->integer &&
		Q_stricmp(filename, "mpdefault.cfg") && Q_stricmp(filename, Q3CONFIG_AUTO) && Q_stricmp(filename, Q3CONFIG_CFG))
	{
		if (f.c && f.c[0] && VALIDSTRING(f.c))
		{
			const char *searchStr = "// generated by Star Wars Jedi Academy MP, do not modify";
			//const char *searchStr = "// generated by EternalJK, do not modify";

			if (!Q_stricmpn(f.c, searchStr, strlen(searchStr))) {
				Com_DPrintf(S_COLOR_YELLOW "Found old/bad header line in cfg file.\n");
				return; //No
			}
		}
	}
